...
__global__ void reduce3(float *y, float *x, int N)
{
    ...
    int block2 = pow2ceil(blockDim.x);
...
int main(int argc, char* argv[])
{
    ...
    // gpu::reduce
    sdkStartTimer(&timer);
    reduce3<<<blocks, threads, threads * sizeof(float)>>>(d_vec2.data().get(), d_vec.data().get(),N);
    reduce3<<<1, blocks, blocks * sizeof(float)>>>(d_vec.data().get(), d_vec2.data().get(), blocks);
