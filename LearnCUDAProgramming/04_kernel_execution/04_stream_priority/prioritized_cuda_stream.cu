...
private:
    ...
    static void CUDART_CB Callback(cudaStream_t stream, cudaError_t status, void *usrData);
    void print_time();
...
void Operator::Callback(cudaStream_t stream, cudaError_t status, void *usrData) {
    Operator *op = (Operator*)usrData;
    op->print_time();
}
